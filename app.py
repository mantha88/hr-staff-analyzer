...
with col1:
    prev_file = st.file_uploader("전월 파일 업로드", type=["xlsx"])

with col2:
    curr_file = st.file_uploader("당월 파일 업로드", type=["xlsx"])
...
if st.button("분석 실행"):
    ...
    join_ids = set(curr_idx.index) - set(prev_idx.index)
    leave_ids = set(prev_idx.index) - set(curr_idx.index)
    common_ids = set(prev_idx.index) & set(curr_idx.index)

    joiners = curr_idx.loc[list(join_ids)].reset_index()
    leavers = prev_idx.loc[list(leave_ids)].reset_index()

    merged = prev_idx.loc[list(common_ids)].add_prefix("전월_") \
        .join(curr_idx.loc[list(common_ids)].add_prefix("당월_"))

    moved = merged[merged["전월_법인"] != merged["당월_법인"]].reset_index()
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        headcount.to_excel(writer, index=False, sheet_name="당월집계")
